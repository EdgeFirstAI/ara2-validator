...
|---|---|---|
| `pybind11/hailo_pybind.cpp` | Replaced the bottom-right-padded `INTER_AREA` letterbox (matching upstream's `make_model_space_canvas`) with an Ultralytics-faithful centered + `(114,114,114)` gray + `INTER_LINEAR` letterbox, and updated the corresponding box-unmap math + `unmap_mask` ROI to use the centered offset. The vendored `upstream/` tree is unchanged. | The `hailo` validator backend exists to be compared head-to-head against the EdgeFirst HAL pipeline. HAL's GPU letterbox is Ultralytics-faithful by cleanroom design; without this fix, the `hailo` row's mAP delta vs HAL conflated preprocess geometry differences with the actual subjects of comparison (NMS algorithm, decoder, mask materialiser). After the fix, both backends consume the same model-input bytes for any given image. |
